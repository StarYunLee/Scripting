...
function readPending(): PendingOAuth | null {
  try {
    ...
    if (!value.state || !value.verifier || !value.createdAt || !value.profileId) return null
...
function identityFromTokens(tokens: TokenPayload): { email: string | null; accountId: string | null } {
  ...
  const email = account?.email_address || account?.email || (typeof jwt?.email === "string" ? jwt.email : null)
  const accountId = account?.uuid || (typeof jwt?.sub === "string" ? jwt.sub : null)
  return { email: email && email.includes("@") ? email : null, accountId: accountId || null }
}
...
function parseAuthorizationInput(input: string, expectedState: string): { code: string; state: string } {
  ...
  if (splitAt > 0) {
    ...
    if (!code || state !== expectedState) throw new Error("OAuth state 校验失败")
    return { code, state }
  }
  if (!/^[A-Za-z0-9._~+/=-]{16,4096}$/.test(value)) throw new Error("Anthropic 授权码格式无效")
  return { code: value, state: expectedState }
}
async function tokenRequest(payload: Record<string, string>): Promise<TokenPayload> {
  const response = await fetch(TOKEN_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", Accept: "application/json", "User-Agent": "anthropic" },
...
function tokenExpiry(tokens: TokenPayload): number {
  if (typeof tokens.expires_at === "number" && Number.isFinite(tokens.expires_at)) {
    return tokens.expires_at > 10_000_000_000 ? tokens.expires_at : tokens.expires_at * 1000
  }
...
/** 创建 Claude Code Authorization Code + PKCE 会话并打开 Anthropic 授权页。 */
...
export async function completeClaudeLogin(callbackOrCode: string): Promise<void> {
...
export async function refreshOAuthToken(profileId: string, force = false): Promise<string | null> {
  ...
  if (!force && current && (!expiresAt || expiresAt > Date.now() + 3 * 60_000)) return current
  const refreshToken = getProfileRefreshToken(profileId)
  if (!refreshToken) return current
  try {
    const tokens = await tokenRequest({ grant_type: "refresh_token", refresh_token: refreshToken, client_id: CLIENT_ID })
    const identity = identityFromTokens(tokens)
