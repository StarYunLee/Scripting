import type { AccountRegistry, ClaudeAccountProfile } from "./types"

const REGISTRY_KEY = "claude_account_registry_v1"
const LEGACY = {
  access: "claude_access_token", refresh: "claude_refresh_token", id: "claude_id_token",
  expires: "claude_token_expires_at", account: "claude_account_id",
}
...
function secretKey(profileId: string, field: string): string { return `claude_profile_${profileId}_${field}` }
...
export function ensureAccountMigration(): AccountRegistry {
  ...
  const profile: ClaudeAccountProfile = { id: makeId(), name: email || "账号 1", email, accountId: getSecretRaw(LEGACY.account), createdAt: now, updatedAt: now }
...
export function listAccounts(): ClaudeAccountProfile[] { return getAccountRegistry().accounts }
export function getDefaultProfileId(): string | null {
  const r = getAccountRegistry()
  return r.defaultAccountId || r.accounts[0]?.id || null
}
export function resolveProfile(profileId?: string | null): ClaudeAccountProfile | null {
  const r = getAccountRegistry()
  if (profileId) {
...
export function createAccount(name = ""): ClaudeAccountProfile {
  const r = getAccountRegistry(); const now = new Date().toISOString()
  const profile: ClaudeAccountProfile = { id: makeId(), name: name.trim() || `账号 ${r.accounts.length + 1}`, email: null, accountId: null, createdAt: now, updatedAt: now }
