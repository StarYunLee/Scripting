...
function modelFor(result: UsageResult): Model {
  ...
  return {
    snapshot,
    fiveHour:
      snapshot?.credits || snapshot?.chat || snapshot?.windows[0] || null,
    weekly:
      snapshot?.chat || snapshot?.completions || snapshot?.windows[1] || null,
    planLabel: snapshot?.planLabel || snapshot?.planType || "GitHub Copilot",
    resetLabel: "重置",
    resetExpiration: formatResetDate(snapshot?.windows[0]?.resetAt || null),
...
export function UsageWidgetView({ result, family }: Props) {
  ...
  if (small) {
    ...
    return (
      ...
      >
        ...
        >
          <PlanBadge provider="copilot" label={model.planLabel} size="small" />
          <Spacer minLength={0} />
          <Text
            fontDesign="default"
            fontWidth="standard"
  ...
  return (
    ...
    >
      ...
      >
        <PlanBadge provider="copilot" label={model.planLabel} />
        <Spacer />
        <Text
          fontDesign="default"
          fontWidth="standard"
