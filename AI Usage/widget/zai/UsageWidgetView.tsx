...
function modelFor(result: UsageResult): Model {
  ...
  return {
    ...
    weekly:
      ...
      snapshot?.windows.find((w) => w.name === "web_search") ||
      snapshot?.windows[1] ||
      null,
    planLabel: snapshot?.planLabel || snapshot?.planType || "Z.ai",
    resetLabel: "重置",
...
export function UsageWidgetView({ result, family }: Props) {
  ...
  if (small) {
    ...
    return (
      ...
      >
        ...
        >
          <PlanBadge provider="zai" label={model.planLabel} size="small" />
          <Spacer minLength={0} />
          <Text
            fontDesign="default"
            fontWidth="standard"
  ...
  return (
    ...
    >
      ...
      >
        <PlanBadge provider="zai" label={model.planLabel} />
        <Spacer />
        <Text
          fontDesign="default"
          fontWidth="standard"
      ...
      />
      <MediumWindow
        title={model.weekly?.name === "weekly" ? "每周额度" : model.weekly?.label || "额度"}
        window={model.weekly}
        width={contentWidth}
        top={96}
