...
function modelFor(result: UsageResult): Model {
  ...
  return {
    ...
    fiveHour:
      snapshot?.auto ||
      snapshot?.windows.find((w) => w.name === "auto") ||
      null,
    weekly:
      snapshot?.total ||
      snapshot?.api ||
      ...
      snapshot?.windows.find((w) => w.name !== "auto") ||
      null,
    planLabel: snapshot?.planLabel || snapshot?.planType || "Cursor",
    resetLabel: "重置",
    resetExpiration: formatResetDate(
      snapshot?.total?.resetAt || snapshot?.api?.resetAt || null,
    ),
...
export function UsageWidgetView({ result, family }: Props) {
  ...
  if (small) {
    ...
    return (
      ...
      >
        ...
        >
          <PlanBadge provider="cursor" label={model.planLabel} size="small" />
          <Spacer minLength={0} />
          <Text
            fontDesign="default"
            fontWidth="standard"
  ...
  return (
    ...
    >
      ...
      >
        <PlanBadge provider="cursor" label={model.planLabel} />
        <Spacer />
        <Text
          fontDesign="default"
          fontWidth="standard"
