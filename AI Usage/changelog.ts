...
export const CHANGELOG = [
  {
    ...
    date: "2026-08-27",
    changes: [
      "优化 App 启动速度：不再每次打开都刷新全部账号，改为按设置里的刷新间隔判断（间隔内直接展示缓存，秒开）。",
      "刷新改为有界并发：「全部刷新」和总览小组件不再逐个串行等待，账号多时明显更快。",
      "启动首帧不再等待账号数据迁移完成。",
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      "修复总用量小组件 Medium 圆环一直转圈：iOS 上 circular ProgressView 会渲染成不确定加载指示器，圆环改用 Circle + trim 手绘，正确显示剩余额度弧。",
