...
const store = createAccountStore<CursorAccountProfile>({
  ...
  migrate: (registry, { getSecret }) => {
    ...
    const accounts = registry.accounts.map((account, index) => {
      const email =
        account.email || tokenEmail(getSecret(account.id, "access_token"));
...
export function updateProfileIdentity(
  ...
  store.update(profileId, (account, index) => {
    ...
    const name =
      email || (placeholder(account.name) ? `账号 ${index + 1}` : account.name);
    return {
      ...account,
...
export function getProfileTokenExpiresAt(
  ...
  const profile = resolveProfile(profileId);
  const value = profile
    ? Number(store.getSecret(profile.id, "expires_at"))
    : NaN;
