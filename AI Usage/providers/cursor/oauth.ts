...
function createUuid(): string {
  const raw = base64Url(Crypto.generateSymmetricKey(192))
    .replace(/[^a-zA-Z0-9]/g, "")
    .padEnd(32, "0")
    .slice(0, 32)
    .toLowerCase();
  return `${raw.slice(0, 8)}-${raw.slice(8, 12)}-4${raw.slice(13, 16)}-${raw.slice(16, 20)}-${raw.slice(20, 32)}`;
}
...
function readPending(): PendingOAuth | null {
  try {
    ...
    if (!value.uuid || !value.verifier || !value.createdAt || !value.profileId)
      return null;
  ...
  for (const headers of attempts) {
    try {
      ...
      if (typeof payload.sub === "string" && payload.sub.trim()) {
        ...
        if (userId && meSub !== userId && jwtSub && meSub !== jwtSub) {
          continue;
        }
      }
...
async function resolveIdentity(
  ...
  const jwt = parseJwtPayload(token);
  let email = emailFromObject(jwt) || emailFromObject(extra || null) || null;
  let accountId =
