...
const REGISTRY_KEY = "grok_account_registry_v1"
const LEGACY = {
  access: "grok_access_token", refresh: "grok_refresh_token", id: "grok_id_token",
  expires: "grok_token_expires_at", account: "grok_account_id",
}
...
function secretKey(profileId: string, field: string): string { return `grok_profile_${profileId}_${field}` }
...
export function ensureAccountMigration(): AccountRegistry {
  ...
  const profile: GrokAccountProfile = { id: makeId(), name: email || "账号 1", email, accountId: getSecretRaw(LEGACY.account), createdAt: now, updatedAt: now }
...
export function listAccounts(): GrokAccountProfile[] { return getAccountRegistry().accounts }
export function getDefaultProfileId(): string | null {
  const r = getAccountRegistry()
  return r.defaultAccountId || r.accounts[0]?.id || null
}
export function resolveProfile(profileId?: string | null): GrokAccountProfile | null {
  const r = getAccountRegistry()
  if (profileId) {
...
export function createAccount(name = ""): GrokAccountProfile {
  ...
  const profile: GrokAccountProfile = { id: makeId(), name: name.trim() || `账号 ${r.accounts.length + 1}`, email: null, accountId: null, createdAt: now, updatedAt: now }
