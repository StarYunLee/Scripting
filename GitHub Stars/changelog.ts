...
export const CHANGELOG = [
  {
    ...
    date: "2026-09-08",
    changes: [
      "修复更换或删除 GitHub 访问令牌后旧账号数据可能重新出现的问题，并在凭据变化时停止旧请求继续分页或重试。",
      "修复取消 Star、重命名列表或调整仓库归属时，旧详情响应可能恢复过期内容的问题。",
      "优化列表归属与 Fork 状态同步，降低网络负载与缓存写入放大。",
      "增强 Stars、本人仓库和列表归属的分页完整性保护，避免截断结果覆盖已有数据。",
    ],
  },
  {
    version: "1.1.0",
